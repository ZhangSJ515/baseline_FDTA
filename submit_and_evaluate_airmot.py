...
def submit_and_evaluate_airmot(config: dict):
    accelerator = Accelerator()
    state = PartialState()

    mode = config["INFERENCE_MODE"]
    if mode not in {"submit", "evaluate"}:
        raise ValueError(f"Unsupported AirMOT inference mode: {mode}")

    inference_dataset = config.get("INFERENCE_DATASET") or "AirMot"
    if inference_dataset not in {"AirMot", "AirMOT"}:
        raise ValueError(
            f"submit_and_evaluate_airmot.py only supports AirMot, got {inference_dataset}."
        )
    ...
    logger.info(
        f"{mode.capitalize()} AirMOT model: {inference_model}, "
    ...
    metrics = submit_and_evaluate_one_airmot_model(
        ...
        filter_gt_by_flag=config.get("AIRMOT_FILTER_GT_BY_FLAG", False),
    )
    ...
    if metrics is not None:
        ...
        logger.metrics(
            log=(
                f"Finish AirMOT evaluation for '{inference_model}' on "
                f"split '{config['INFERENCE_SPLIT']}': "
            ),
            metrics=metrics,
...
def submit_and_evaluate_one_airmot_model(
        is_evaluate: bool,
        accelerator: Accelerator,
        state: PartialState,
        logger: Logger,
    ...
    if not sequence_names:
        raise RuntimeError("No AirMOT sequences are available for inference.")
    ...
    for sequence_name in selected_sequence_names:
        ...
        runtime_tracker = AirMOTRuntimeTracker(
            model=model,
        ...
        logger.info(
            f"Submitting AirMOT sequence {sequence_name} with "
        ...
        if not is_fake_process:
            ...
            for frame_index, frame_result in enumerate(sequence_results):
                ...
                for obj_id, score, category, bbox in zip(
                        frame_result["id"],
                        frame_result["score"],
                        frame_result["category"],
                        frame_result["bbox"],
                ):
                    rows.append(
                        f"{frame_id} {int(obj_id.item())} {int(category.item())} "
                        f"{float(bbox[0].item()):.6f} {float(bbox[1].item()):.6f} "
                        f"{float(bbox[2].item()):.6f} {float(bbox[3].item()):.6f} "
                        f"{float(score.item()):.6f}\n"
                    )
            ...
            logger.success(
                f"AirMOT sequence {sequence_name} saved to {result_path}; "
                f"FPS: {sequence_fps:.2f}.",
                only_main=False,
            )
        else:
            logger.success(
                f"Fake AirMOT sequence {sequence_name} completed; FPS: {sequence_fps:.2f}.",
                only_main=False,
            )

    accelerator.wait_for_everyone()
    if not is_evaluate:
        logger.success(f"AirMOT submission files saved to {tracker_dir}.")
        return None

    if accelerator.is_main_process:
        gt_dir = os.path.join(data_root, "AirMot", data_split)
        if not os.path.isdir(gt_dir):
            alternate_gt_dir = os.path.join(data_root, "AirMOT", data_split)
            if os.path.isdir(alternate_gt_dir):
                gt_dir = alternate_gt_dir

        current_dir = os.path.dirname(os.path.abspath(__file__))
        trackeval_script = os.path.join(
            current_dir, "TrackEval", "scripts", "run_airmot.py"
        )
        command = [
            "python",
            trackeval_script,
            ...
            "--TRACKER_SUB_FOLDER", "",
            "--OUTPUT_SUB_FOLDER", "",
            "--CLASSES_TO_EVAL", *AIRMOT_CLASSES,
            "--METRICS", "HOTA", "CLEAR", "Identity",
            "--USE_PARALLEL", "True",
            "--NUM_PARALLEL_CORES", "8",
            "--PLOT_CURVES", "False",
            "--PRINT_ONLY_COMBINED", "True",
            "--FILTER_GT_BY_FLAG", str(bool(filter_gt_by_flag)),
        ]
        logger.info("Start native multi-class AirMOT evaluation.")
        completed = subprocess.run(command, check=False)
        if completed.returncode != 0:
            raise RuntimeError(
                f"AirMOT TrackEval failed with code {completed.returncode}."
            )
        logger.success("AirMOT TrackEval completed.")

    accelerator.wait_for_everyone()
    metric_path = os.path.join(tracker_dir, "cls_comb_det_av_summary.txt")
    ...
    logger.success(f"Loaded AirMOT metrics from {metric_path}.")
    return metrics
...
if __name__ == "__main__":
    ...
    submit_and_evaluate_airmot(config=config)
